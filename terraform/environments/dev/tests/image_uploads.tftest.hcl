...
run "pending_expiration_and_private_cdn" {
  ...
  assert {
    condition = alltrue([
      ...
      statement.sid != "AllowUploadsCloudFrontRead" || (
        ...
        toset(statement.resources) == toset([
          ...
          "${aws_s3_bucket.app["uploads"].arn}/claims/*",
        ]) &&
        alltrue([
    ...
    error_message = "TLS 강제와 해당 배포의 reviews/profiles/claims 조회 권한을 하나의 버킷 정책에 보존해야 합니다."
  }
  ...
  assert {
    condition = alltrue(flatten([
      for name, document in data.aws_iam_policy_document.image_upload : [
        for statement in document.statement :
        toset(statement.resources) == toset([
          "arn:aws:s3:::petflow-dev-uploads/${name == "review-service" ? "reviews" : name == "member-service" ? "profiles" : "claims"}/*",
          ]) && alltrue([
